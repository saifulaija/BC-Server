...
//   console.log(req.body.admin,'admin.............')
...
//     const url = req.body.profilePhoto;
...
const createSubscriber = async (req: Request): Promise<Patient> => {
  const file = req.file as IUploadFile;

  if (file) {
    const uploadedProfileImage =
      await FileUploadHelper.uploadToCloudinary(file);
    req.body.subscriber.profilePhoto = uploadedProfileImage?.secure_url;
  }
  ...
  const result = await prisma.$transaction(async transactionClient => {
    ...
    const newPatient = await transactionClient.subscriber.create({
      data: req.body.subscriber,
    });
